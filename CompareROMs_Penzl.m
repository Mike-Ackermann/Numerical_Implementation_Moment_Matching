%Plots errors in Loewner, Hermite Loewner, VF, and AAA approximations using
...
%% Construct 4 ROMS from estimated data
...
[~,idx] = sort(zz(1:length(z)),'ComparisonMethod','real');
zz = zz([idx;idx+length(z)]);
HzHz = HzHz([idx;idx+length(z)]);

[Ap2,Bp2,Cp2,Ep2] = Loewner(zz,HzHz,nmax);
%Vector Fitting
...
%AAA
[R, pol, res, zer, zj, fj, wj, errvec, wt] = aaa(Hz_WC(:,1), z_WC, 'degree',nmax);
[Aaaa, Baaa, Caaa, Eaaa] = MakeAAASys(zj,wj,fj);
...
%Loewner
[Ap2t,Bp2t,Cp2t,Ep2t] = Loewner(zz,HzHz_true,nmax);
%Vector Fitting
count_VFt = 0;
converged = false; diverged = false;
polest = initl_poles;
...
%AAA
[Rt, polt, rest, zert, zjt, fjt, wjt, errvect, wtt] = aaa(H_interp_true, z_WC, 'degree',nmax);
[Aaaat, Baaat, Caaat, Eaaat] = MakeAAASys(zjt,wjt,fjt);
...
sysd_AAA = sparss(Aaaa,Baaa,Caaa,0,Eaaa,Ts);
...
sysd_AAAt = sparss(Aaaat,Baaat,Caaat,0,Eaaat,Ts);

%AAA can produce unstable systems.  If this happens, take only the stable
%part.
if max(abs(eig(full(sysd_AAA)))) >= 1
    sysd_AAA_full = sysd_AAA;
    [sysd_AAA, Gus] = stabsep(full(sysd_AAA));
    fprintf('AAA was unstable!!!\n')
    fprintf('Stable AAA dim: %d\n',length(sysd_AAA.A))
...
%freqs_used = logspace(log_min_freq,pi,num+1)/Ts; %VF uses frequency, not points in C
%freqs_used = freqs_used(1:end-1);
freqs_used = freqs/Ts;
...
H_AAA = freqresp(sysd_AAA,freqs_plot); H_AAA = squeeze(H_AAA);
...
H_AAAt = freqresp(sysd_AAAt,freqs_plot); H_AAAt = squeeze(H_AAAt);
...
err_AAA = abs(H_AAA - H_true)./abs(H_true);
%err_interp = abs(Hz(:,1)-H_interp_true)./abs(H_interp_true);
err_interp = abs(Hz(:,1)-H_true_used)./abs(H_true_used);
%ROMs from true data
err_Lowt = abs(H_Lowt - H_true)./abs(H_true);
err_HerLowt = abs(H_HerLowt - H_true)./abs(H_true);
err_VFt = abs(H_VFt - H_true)./abs(H_true);
err_AAAt = abs(H_AAAt - H_true)./abs(H_true);
%% plot bode plot from estimated data (only Loewner models)
%plot bode plot from estimated data
%need to make freqs be in [-log_min_freq, pi)
freqs_plt = freqs_plot*Ts;
figure
loglog(freqs_plt,abs(H_true),'LineWidth',2)
hold on
%%%%%%%loglog(freqs_plt, abs(H_Low),'--','LineWidth',2)
loglog(freqs_plt, abs(H_HerLow),':','LineWidth',2)
loglog(freqs_plt, abs(H_Low),'-.','LineWidth',2)
legend('$H$','$\tilde H_l^{HL}$','$\tilde H_l^{L}$',...
    'interpreter','latex')
%title('freq resp estimated data')
...
xticklabels({'10^{-3}','10^{-1}','\pi'})
xlabel('$\omega$','interpreter','latex','fontsize',25)
%set limits of plot
xlim([1e-3,pi])
%labels
ylabel('$|\tilde H_l^x(e^{\mathbf i \omega})|$','interpreter','latex','fontsize',20)
% xlabel('$r$','interpreter','latex','fontsize',30)
lgd = legend();
lgd.Location = 'northwest';

%% plot errors from estiamted data (only Loewner models)
...
xlim([1e-3,pi])
%labels
ylabel('$|H(e^{\mathbf i \omega})-\tilde H_l^x(e^{\mathbf i \omega})|/|H(e^{\mathbf i \omega})|$','interpreter','latex','fontsize',20)
% xlabel('$r$','interpreter','latex','fontsize',30)
lgd = legend();
lgd.Location = 'northwest';
...
%%%%%%%loglog(freqs_plt, abs(H_Low),'--','LineWidth',2)
loglog(freqs_plt, abs(H_HerLow),':','LineWidth',2)
loglog(freqs_plt, abs(H_VF),'-.','LineWidth',2)
loglog(freqs_plt, abs(H_AAA),'-.','LineWidth',2)
%%%%%%%legend('HTrue','HLOw','HHerLow','HVF','HAAA','IRKA')
%legend('HTrue','HHerLow','HVF','HAAA','IRKA')
legend('$H$','$\tilde H_l^{HL}$','$\tilde H_l^{VF}$',...
    '$\tilde H_l^{AAA}$','interpreter','latex')
%title('freq resp estimated data')
...
loglog(freqs_plt,abs(H_true),'LineWidth',2)
hold on
%loglog(freqs_plt, abs(H_Lowt),'--','LineWidth',2)
loglog(freqs_plt, abs(H_HerLowt),':','LineWidth',2)
loglog(freqs_plt, abs(H_VFt),'-.','LineWidth',2)
loglog(freqs_plt, abs(H_AAAt),'-.','LineWidth',2)
%%%%%%legend('HTrue','HLOw','HHerLow','HVF','HAAA','IRKA')
%legend('HTrue','HHerLow','HVF','HAAA','IRKA')
legend('$H$','$\tilde H^{HL}$','$\tilde H^{VF}$',...
    '$\tilde H^{AAA}$','interpreter','latex')
%title('freq resp true data')
ax = gca;
Default_TW = ax.TickLength;
Default_LW = ax.LineWidth;
%double tick width and length
...
xlim([1e-3,pi])
%labels
ylabel('$|\tilde H^x(e^{\mathbf i \omega})|$','interpreter','latex','fontsize',20)
lgd = legend();
lgd.Location = 'northwest';
%% plot errors from estiamted data
%plot errors from estiamted data
...
hold on
%loglog(freqs_plt, err_Low,'LineWidth',2)
%loglog(freqs_plt_used,err_interp,'LineWidth',2)
loglog(freqs_plt, err_VF,'LineWidth',2)
loglog(freqs_plt, err_AAA,'LineWidth',2)
%%%%%%%legend('HerLow','Low','Interp','VF','AAA','IRKA')
%legend('HerLow','VF','AAA','IRKA')
legend('$\tilde H_l^{HL}$','$\tilde H_l^{VF}$',...
    '$\tilde H_l^{AAA}$','interpreter','latex')
%title('Relative error at random points on unit circle estiamted data')
...
loglog(freqs_plt, err_AAAt,'LineWidth',2)
%legend('HerLow','Low','VF','AAA','IRKA')
%legend('HerLow','VF','AAA','IRKA')
%title('Relative error at random points on unit circle true data')
legend('$\tilde H^{HL}$','$\tilde H^{VF}$',...
    '$\tilde H^{AAA}$','interpreter','latex')
...
xlim([1e-3,pi])
%labels
ylabel('$|H(e^{\mathbf i \omega})-\tilde H^x(e^{\mathbf i \omega})|/|H(e^{\mathbf i \omega})|$','interpreter','latex','fontsize',20)
% xlabel('$r$','interpreter','latex','fontsize',30)
lgd = legend();
lgd.Location = 'northwest';
%% Plot other stuff

% %plot Loewner error for measured vs true data
% figure
% loglog(freqs_plt,err_Low,'LineWidth',2)
% hold on
% loglog(freqs_plt,err_Lowt,'LineWidth',2)
% legend('Low from measured','Low from true')
% title('comparison of error at random points for loewner from true and measured data')
% 
% %plot Hermite Loewner error for measured vs true data
% figure
% loglog(freqs_plt,err_HerLow,'LineWidth',2)
% hold on
% loglog(freqs_plt,err_HerLowt,'--','LineWidth',2)
% legend('HerLow from measured','HerLow from true')
% title('comparison of error at random points for Her loewner from true and measured data')
% 
% %plot Vector Fitting error for measured vs true data
% figure
% loglog(freqs_plt,err_VF,'LineWidth',2)
% hold on
% loglog(freqs_plt,err_VFt,'--','LineWidth',2)
% legend('VF from measured','VF from true')
% title('comparison of error at random points for VF from true and measured data')
% 
% %plot AAA error for measured vs true data
% figure
% loglog(freqs_plt,err_AAA,'LineWidth',2)
% hold on
% loglog(freqs_plt,err_AAAt,'--','LineWidth',2)
% legend('AAA from measured','AAA from true')
% title('comparison of error at random points for AAA from true and measured data')
% 
% figure
% loglog(freqs_plt,err_IRKA,'LineWidth',2)
% hold on
% loglog(freqs_plt,err_IRKAt,'--','LineWidth',2)
% legend('TD-IRKA','TF-IRKA')
% title('comparison of error at random points for IRKA from true and measured data')
...
H2_dist_AAA = norm(full(sysd_AAA)-full(sysd_AAAt))/norm(full(sysd_AAAt));
...
Hinf_AAA = norm(sysd-full(sysd_AAA),'inf')/Hinf_Sysd;
...
Hinf_AAAt = norm(sysd-full(sysd_AAAt),'inf')/Hinf_Sysd;
...
Hinf_dist_AAA = norm(full(sysd_AAA)-full(sysd_AAAt),'inf')/norm(full(sysd_AAAt),'inf');

%% Output Error norm results

fprintf('------ H2 FROM APX ERRORS ------\n')
fprintf('Low: %f, HerLow: %f, VF: %f, AAA: %f',...
    H2_Low, H2_HerLow, H2_VF, H2_AAA)
fprintf('------ H2 FROM TRUE ERRORS ------\n')
fprintf('Low: %f, HerLow: %f, VF: %f, AAA: %f',...
    H2_Lowt, H2_HerLowt, H2_VFt, H2_AAAt)
...
H2_err_mat = [H2_HerLow, H2_VF, H2_AAA;...
              H2_HerLowt, H2_VFt, H2_AAAt;...
              H2_dist_HerLow, H2_dist_VF, H2_dist_AAA];

Hinf_err_mat = [Hinf_HerLow, Hinf_VF, Hinf_AAA;
                Hinf_HerLowt, Hinf_VFt, Hinf_AAAt;...
                Hinf_dist_HerLow, Hinf_dist_VF, Hinf_dist_AAA];

%matrix2latex(H2_err_mat, 'H2_err_RandEx1_correct.txt')
