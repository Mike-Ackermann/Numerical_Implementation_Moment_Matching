function [SER,poles,rmserr,fit,opts]=vectfit3_discrete(f,s,poles,weight,Ts,opts);
%
...
def.asymp=1;      %Include only D in fitting (not E)   %% ZD changed 2 to 1   opts.asymp=1 --> Fitting with D=0,  E=0
...
def.errplot=0;    %Include deviation in magnitude plot  %% ZD changed 1 to 0
...
%=========================================================================
%=========================================================================
s = exp(s*Ts*1i); % map imaginary axis to unit circle
...
if opts.skip_pole~=1
    ...
    %=============================================
    %=============================================
    unstables=abs(roetter)>1;
    if opts.stable==1
        %roetter(unstables)=roetter(unstables)-2*real(roetter(unstables));
        roetter(unstables)=1./roetter(unstables);%Forcing unstable poles to be stable...
    end
    roetter=sort(roetter); N=length(roetter);
    
    
    %=============================================
    %Sort the poles such that the real ones come first:
    for n=1:N
        for m=n+1:N
